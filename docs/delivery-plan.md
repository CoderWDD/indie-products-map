...
- schema 覆盖项目、AI 分析、链接状态、官网摘要、更新日志。`patterns.json` 和 `unclustered-projects.json` 作为历史兼容空文件保留校验。
...
- 搜索范围覆盖项目名、作者、原始简介、AI 摘要、标签。
- 支持目标用户、产品类型、商业化方式多选筛选。
- 同一维度内 OR，不同维度之间 AND。
- 搜索结果再应用筛选。
...
- 首页、项目详情页、关于页有基础 meta。
- 项目详情页 canonical 稳定。
- sitemap 包含首页、项目详情页、关于页。
